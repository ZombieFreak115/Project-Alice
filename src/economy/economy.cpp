			...
			for(uint32_t i = 0; i < commodity_set::set_size; ++i) {
				if(build_cost.commodity_type[i]) {
					...
					if(reinforcement > 0) {
						...
						// request only a 1/30 of the build cost per day, so it adds up to the full build cost in total
						state.world.market_set_army_demand(market, build_cost.commodity_type[i], curr_demand + (build_cost.commodity_amounts[i] / 30.0f ) * reinforcement);
					}
				} else {
					break;
				}
...
void daily_update(sys::state& state, bool presimulation, float presimulation_stage) {
	...
	*/
	...
	state.world.execute_parallel_over_pop([&](auto ids) {
		...
		if(presimulation) {
			base_income = pop_of_type * expected_savings_per_capita * (1.f - presimulation_stage);
		} else {
			// miniscule base income to counteract deflation spiral
			base_income = pop_of_type * expected_savings_per_capita * 0.01f;
		}
		...
		}
		{
			...
			state.world.nation_for_each_state_ownership_as_nation(n, [&](dcon::state_ownership_id soid) {
				...
				for(uint32_t k = 1; k < total_commodities; ++k) {
					...
					state.world.nation_set_army_stockpile(n, c, current + std::min(sat, val));
				}
			...
			for(auto ar : state.world.nation_get_army_control(n)) {
				for(auto reg : state.world.army_get_army_membership(ar.get_army().id)) {
					...
					for(uint32_t com = 0; com < commodity_set::set_size; com++) {
						...
						float actual_needed = unit_supply_full * supply_consumption_mod; // * (state.world.nation_get_land_spending(n) / 100);
						...
						if(current_stockpile_inventory >= actual_needed) {
							...
							supplied += current_stockpile_inventory;
			...
			for(auto ar : state.world.nation_get_army_control(n)) {
				for(auto reg : state.world.army_get_army_membership(ar.get_army().id)) {
					...
					for(uint32_t com = 0; com < commodity_set::set_size; com++) {
						dcon::commodity_id item = state.military_definitions.unit_base_definitions[reg.get_regiment().get_type()].build_cost.commodity_type[com];
						float unit_supply_full = state.military_definitions.unit_base_definitions[reg.get_regiment().get_type()].build_cost.commodity_amounts[com];
						if(unit_supply_full == 0.0f) {
							continue;
						}
						auto& current_stockpile_inventory = state.world.nation_get_army_stockpile(n, item);
						float actual_needed = unit_supply_full; // * (state.world.nation_get_land_spending(n) / 100);
						total_supply += unit_supply_full;
						if(current_stockpile_inventory >= actual_needed) {
							supplied += actual_needed;
							state.world.nation_set_army_stockpile(n, item, current_stockpile_inventory - actual_needed);
