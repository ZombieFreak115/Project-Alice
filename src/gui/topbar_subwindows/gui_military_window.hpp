	...
	void on_update(sys::state& state) noexcept override {
		...
		for(const auto ac : state.world.nation_get_army_control_as_controller(n)) {
			for(const auto am : ac.get_army().get_army_membership()) {
				if(am.get_regiment().get_regiment_source().begin() != am.get_regiment().get_regiment_source().end()) {
					// only check first since it is mobilized
					auto pop = *am.get_regiment().get_regiment_source().begin();
					if(pop.get_pop().get_poptype() != state.culture_definitions.soldiers
						...
						cur_mobilization += int32_t(pop.get_pop().get_size() * military::mobilization_size(state, n) / state.defines.pop_size_per_regiment);
					}
				}
...
public:
	void on_update(sys::state& state) noexcept override {
		...
		for(const auto ac : state.world.nation_get_army_control_as_controller(n)) {
			for(const auto am : ac.get_army().get_army_membership()) {
				if(am.get_regiment().get_regiment_source().begin() != am.get_regiment().get_regiment_source().end()) {
					// only check first connected pop cause its mobilized
					auto pop = *am.get_regiment().get_regiment_source().begin();
					if(military::pop_eligible_for_mobilization(state, pop.get_pop())) {
						cur_mobilization += int32_t(pop.get_pop().get_size() * military::mobilization_size(state, n) / state.defines.pop_size_per_regiment);
					}
				}
