...
inline void province_building_effect_tooltip(sys::state& state, text::columnar_layout& contents, dcon::province_id p, dcon::province_building_type_id bt, float level = 1.f) {
	modifier_description(state, contents, state.world.province_building_type_get_modifiers(bt), 0, level);
	auto type = state.world.province_building_type_get_type(bt);
	// Since trade accounts for naval bases level separately, show special case for trade attractiveness for them
	if(type == uint8_t(economy::province_building_type::naval_base)) {
		auto box = text::open_layout_box(contents, 0);
	...
	else if(type == uint8_t(economy::province_building_type::railroad)) {
		auto box = text::open_layout_box(contents, 0);
		text::add_to_layout_box(state, contents, box, text::produce_simple_string(state, "infrastructure"), text::text_color::white);
		text::add_to_layout_box(state, contents, box, std::string_view{ ":" }, text::text_color::white);
		text::add_space_to_layout_box(state, contents, box);
		text::add_to_layout_box(state, contents, box, text::fp_percentage{ state.world.province_building_type_get_infrastructure(bt) }, text::text_color::green);
		text::close_layout_box(contents, box);
	}
}

inline void province_building_tooltip(sys::state& state, text::columnar_layout& contents, dcon::province_id p, dcon::province_building_type_id bt) {
	auto level = state.world.province_get_building_level(p, bt);

	text::add_line(state, contents, state.world.province_building_type_get_name(bt));
...
inline void province_building_construction_tooltip(sys::state& state, text::columnar_layout& contents, dcon::province_id p, dcon::province_building_type_id bt) {
	text::add_line(state, contents, state.world.province_building_type_get_name(bt));
	text::add_line_break_to_layout(state, contents);
	...
	int32_t current_lvl = state.world.province_get_building_level(p, bt);
	int32_t max_local_lvl = state.world.nation_get_max_building_level(state.local_player_nation, bt);
	uint8_t type = state.world.province_building_type_get_type(bt);
	if (type == uint8_t(economy::province_building_type::fort)) {
	...
	auto constr_cost = state.world.province_building_type_get_cost(bt);
