...
std::vector<uint32_t> supply_loss_map_from(sys::state& state) {
	uint32_t province_size = state.world.province_size();
	uint32_t texture_size = province_size + 256 - province_size % 256;
	...
	if(for_nation) {
		...
		for(auto p : state.world.nation_get_province_ownership(for_nation)) {
			auto v = supply_routes::province_supply_attrition_modifier(state, p.get_province(), state.local_player_nation);
			mn = std::min(mn, v);
			mx = std::max(mx, v);
		}
		if(mx > mn) {
		...
		province::for_each_land_province(state, [&](dcon::province_id prov) {
			auto v = supply_routes::province_supply_attrition_modifier(state, prov, state.local_player_nation);
			mn = std::min(mn, v);
			mx = std::max(mx, v);
		});
		if(mx > mn) {
...
std::vector<uint32_t> supply_throughput_map_from(sys::state& state) {
	uint32_t province_size = state.world.province_size();
	uint32_t texture_size = province_size + 256 - province_size % 256;
	...
	if(for_nation) {
		...
		float mn = 1.0f;
		for(auto p : state.world.nation_get_province_ownership(for_nation)) {
			auto v = supply_routes::supply_throughput_modifier(state, p.get_province(), state.local_player_nation);
			mn = std::min(mn, v);
			mx = std::max(mx, v);
		}
		if(mx > mn) {
		...
		float mn = 1.0f;
		province::for_each_land_province(state, [&](dcon::province_id prov) {
			auto v = supply_routes::supply_throughput_modifier(state, prov, state.local_player_nation);
			mn = std::min(mn, v);
			mx = std::max(mx, v);
		});
		if(mx > mn) {
...
// Comput the gradient of 0..1 of throughput satisfaction to display as color.
std::vector<uint32_t> supply_route_efficiency_map_from(sys::state& state) {
	...
	if(for_nation) {
		...
		float mn = 1.0f;
		for(auto p : state.world.nation_get_province_ownership(for_nation)) {
			auto eff = supply_routes::supply_throughput_efficiency(state, p.get_province(), state.local_player_nation);
			mn = std::min(mn, eff);
			mx = std::max(mx, eff);
		}
		if(mx > mn) {
			for(auto p : state.world.nation_get_province_ownership(for_nation)) {
				...
				uint32_t color = ogl::color_gradient((eff - mn) / (mx - mn),
					sys::pack_color(46, 247, 15),	// to green
					sys::pack_color(247, 15, 15)	// from red
				);
				auto i = province::to_map_id(p.get_province());
		...
		float mn = 1.0f;
		province::for_each_land_province(state, [&](dcon::province_id prov) {
			auto eff = supply_routes::supply_throughput_efficiency(state, prov, state.local_player_nation);
			mn = std::min(mn, eff);
			mx = std::max(mx, eff);
		});
		if(mx > mn) {
...
void set_map_mode(sys::state& state, mode mode) {
	...
	case mode::workforce:
		...
		break;
	case mode::supply_loss:
		prov_color = supply_loss_map_from(state);
		break;
